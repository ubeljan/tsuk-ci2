...
document.observe('dom:loaded', function(){
	Sortable.create('divContainer',{
		...
		ajax('<?php echo base_url(); ?>_ajax/sort_categories', 'order_list', poststr, 'POST');
		},tag: 'div'
	});
});
...
function refresh(){
	parent.location="<?php echo $this->config->item('app_pad') ; ?>admin/categories";	
}
...
<?php
echo '<div style="text-align:left; margin-bottom:5px">Admin&nbsp;&gt;&nbsp;<strong>Categories</strong></div>';
?>
...
if ($cats==1){	
	foreach ($category as $category_info) {
		...
		echo '<tr><td>Title:&nbsp;<input type="text" id="title_'.$category_info['key'].'" class="textstyle" style="width:250px" value="'.$category_info['title'].'">&nbsp;<input type="button" class="buttonstyle" value="Save" onclick="save_category(\''.$category_info['key'].'\',\'edit\');">&nbsp;<input type="button" class="buttonstyle" value="Delete" onclick="save_category(\''.$category_info['key'].'\',\'delete\');">&nbsp;<input type="button" class="buttonstyle" value="Subcategories" onclick="javascript:parent.location=\''.url().'admin/categories/'.$category_info['key'].'\';"></td><td><img src="'.url().'images/icon_drag.png" alt="Move"><span id="status_'.$category_info['key'].'" style="display:none"></span></td></tr>';
		echo '</table>';
		echo '</div>';
		$i++;
	}
